...
export function useWhatsAppConnections() {
  ...
  const {
    ...
    queryFn: async () => {
      console.log(
        "Fetching WhatsApp connections for workspace:",
        workspace?.id
      );
      if (!workspace?.id) {
        console.warn("No workspace ID found for fetch");
        return [];
      }

      console.log("Executing Supabase query on 'whatsapp_connections'...");
      ...
      console.log("Successfully fetched connections count:", data?.length || 0);
    ...
    try {
      ...
      if (data?.status === "connected") {
        ...
        toast({
          ...
          description:
            'Sua conta do Meta Business não tem número de telefone WhatsApp configurado. Use a opção "Conectar via QR Code" para conectar diretamente.',
          duration: 8000,
        });
        return true;
      }
  ...
  const startOAuthPolling = useCallback(
    (connectionId: string, popup: Window) => {
        ...
        if (oauthStateRef.current.popup?.closed) {
          ...
          if (!wasSuccessful) {
            toast({
              ...
              description:
                "A janela de autenticação foi fechada. Tente novamente.",
              variant: "destructive",
            });
          }
      ...
      if (data?.oauth_url) {
        ...
        if (popup && !popup.closed && data?.connection_id) {
          ...
          toast({
            title: "Autenticação Facebook",
            description:
              "Complete a autenticação na nova aba. Retorne aqui após concluir.",
          });
        }
        return;
      }
  ...
  const checkStatus = useMutation({
      ...
      if (data?.status === "connected") {
        toast({
          ...
          description: `WhatsApp conectado com sucesso. ${
            data.phone_number ? `Número: ${data.phone_number}` : ""
          }`,
        });
      } else {
        toast({
          title: "Status verificado",
          description: `A conexão está em estado: ${
    ...
    onError: (error: any) => {
      ...
      toast({
        ...
        description:
          error.message || "Ocorreu um erro ao verificar o status da conexão.",
        variant: "destructive",
      });
    },
  });
